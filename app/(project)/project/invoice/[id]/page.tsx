   ...
   <div className="max-w-2xl mx-auto p-8">
      <div className="flex justify-between items-center py-3">
         <Button
            ...
            className="mb-4"
          >
            <a href={`dashboard/${project}`} className="flex items-center">
      ...
      <div className="p-4 sm:p-7 overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm font-medium dark:text-neutral-500 pt-3">
            Invoice Date: {" "}  <span className="text-muted-foreground">
        ...
        <div className="mt-5 sm:mt-10 grid grid-cols-2 sm:grid-cols-3 gap-5">
          <div>
            ...
            <span className="block text-sm font-medium text-gray-800 dark:text-neutral-200">{invoiceDetails?.invoice.amount.toLocaleString()}</span>
          </div>
        ...
        <div className="mt-5 sm:mt-10">
          <p className="text-sm text-gray-500 dark:text-neutral-500">If you have any questions, please contact us at <a className="inline-flex items-center gap-x-1.5 text-blue-600 decoration-2 hover:underline focus:outline-hidden focus:underline font-medium dark:text-blue-500" href={`mailto:${invoiceDetails?.user?.email}`}>{invoiceDetails?.user?.email}</a> or call at <a className="inline-flex items-center gap-x-1.5 text-blue-600 decoration-2 hover:underline focus:outline-hidden focus:underline font-medium dark:text-blue-500" href={`tel:${invoiceDetails?.user?.phone}`}>{invoiceDetails?.user?.phone}</a></p>
        </div>
      </div>
    </div>
   </div>
